...
class _TrackingScreenState extends State<TrackingScreen> {
    ...
    if (data.length > 0) {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: const Text("Nie pobrano snu"),
            content: const Text(
                "Nie udało się pobrać danych snu dla wyznaczonej daty."),
            actions: <Widget>[
              TextButton(
  ...
  void dateForward() async {
    DateTime currentDate = _entries[0].date!;
    if (currentDate.add(const Duration(days: 1)).compareTo(DateTime.now()) <=
        0) {
      getDataForDay(currentDate.add(const Duration(days: 1)));
    } else {
  ...
  void dateBackward() async {
    getDataForDay(_entries[0].date!.subtract(const Duration(days: 1)));
  }
        ...
        GestureDetector(
          ...
          child: Scaffold(
              ...
              floatingActionButton:
                  _entries[0].date!.year != DateTime.now().year ||
                          _entries[0].date!.month != DateTime.now().month ||
                          _entries[0].date!.day != DateTime.now().day
                      ? FloatingActionButton(
              ...
              body: RefreshIndicator(
                key: _refreshState,
                onRefresh: () async {
                  await getDataForDay(_entries[0].date!);
                },
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        Expanded(
                          child: TextButton(
                            ...
                            child: Text(dateToDateString(_entries[0].date!)),
                          ),
                        ),
                        Material(
                          child: IconButton(
                          ...
                          return Column(
                            children: [
                              ...
                              ListTile(
                                ...
                                onTap: () async {
                                  DateTime? selectedTime = await askForTime();
                                  if (selectedTime != null) {
                                    await updateEntry(
                                        _entries[index].date!,
                                        TrackingEntry(bedTime: selectedTime)
                                            .toMapWithoutDate());
                                  }
                                },
                                subtitle: Text(_entries[index].bedTime == null
                                    ? "-"
                                    : dateToTimeString(_entries[index].bedTime!,
                                        excludeSeconds: true)),
                                trailing: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                              ...
                              ListTile(
                                ...
                                onTap: () async {
                                  ...
                                  if (selectedTime != null) {
                                    await updateEntry(
                                        _entries[index].date!,
                                        TrackingEntry(sleepTime: selectedTime)
                                            .toMapWithoutDate());
                                  }
                                },
                                subtitle: Text(_entries[index].sleepTime == null
                                    ? "-"
                                    : dateToTimeString(
                                ...
                                onTap: () async {
                                  ...
                                  if (selectedTime != null) {
                                    await updateEntry(
                                        _entries[index].date!,
                                        TrackingEntry(
                                                firstAlarmTime: selectedTime)
                                            .toMapWithoutDate());
                                  }
                                },
                                subtitle: Text(
                                    _entries[index].firstAlarmTime == null
                                        ? "-"
                                        : dateToTimeString(
                              ...
                              ListTile(
                                ...
                                onTap: () async {
                                  ...
                                  if (selectedTime != null) {
                                    await updateEntry(
                                        _entries[index].date!,
                                        TrackingEntry(wakeUpTime: selectedTime)
                                            .toMapWithoutDate());
                                  }
                                },
                                subtitle: Text(
                              ...
                              ListTile(
                                ...
                                onTap: () async {
                                  ...
                                  if (selectedTime != null) {
                                    await updateEntry(
                                        _entries[index].date!,
                                        TrackingEntry(getUpTime: selectedTime)
                                            .toMapWithoutDate());
                                  }
                                },
                                subtitle: Text(_entries[index].getUpTime == null
                                    ? "-"
                                    : dateToTimeString(
                              ...
                              ListTile(
                                ...
                                onTap: () async {
                                  ...
                                  bool? change = await showDialog(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        title: const Text("Wybierz ocenę"),
                                        content: SelectNumberSlider(
                                          min: 0,
                                          max: 10,
                                          divisions: 10,
                                          init: rate,
                                          onSelect: (int val) => rate = val,
                                        ),
                                        actions: <Widget>[
                                          TextButton(
                                            onPressed: () =>
